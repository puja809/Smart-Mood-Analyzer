...
async function uploadToS3(file) {
    ...
    await s3.upload(params).on('httpUploadProgress', (evt) => {
        ...
        // Fetch the file contents based on the uploaded picture name
        fetchFileFromS3('my-rek-image-parsed-data', file.name);
    });
}

// Gemini API Set
const API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=AIzaSyDmqqLaM1M_iDj0vL6NGXoLI_5iC2wCsCY`;
...
await wait(10000);
    await s3.listObjectsV2(params, (error, data) => {
        ...
        const regex = new RegExp(`^${folderPath}.*${pictureName.split('.')[0]}.*\\.txt$`, 'i'); // Match within folder and by base name
